...
use {
  self::subcommand::wallet::inscribe::Payout,
  super::*,
  std::cmp::{max, min},
};
    ...
    if let Some(excess) = value.checked_sub(self.fee_rate.fee(self.estimate_vbytes())) {
              ...
              .fee(self.estimate_vbytes() + Self::ADDITIONAL_OUTPUT_VBYTES)
      {
        ...
        for payout in self.payouts.clone() {
          self
            .outputs
            .push((payout.destination.clone(), payout.amount));
        }
...
mod tests {
  ...
  #[test]
  #[should_panic(expected = "invariant: all outputs are either change, recipient, or payout")]
